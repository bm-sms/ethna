...
class Ethna_Plugin_Generator_Test extends Ethna_Plugin_Generator
{
    ...
    function &generate($skelfile, $name)
    {
        ...
        // ファイル名生成
        $file = preg_replace_callback('/_(.)/', function ($m) { return '/' . strtoupper($m[1]); }, ucfirst($name)) . "Test.php";
        ...
        $macro['name'] = preg_replace_callback('/_(.)/', function ($m) { return strtoupper($m[1]); }, ucfirst($name));
