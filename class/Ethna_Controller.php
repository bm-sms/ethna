...
class Ethna_Controller
{
    ...
    function _trigger_WWW($default_action_name = "", $fallback_action_name = "")
    {
        ...
        if ($forward_name != null) {
            ...
            $this->view = $this->createView($view_class_name, $backend, $forward_name, $this->_getForwardPath($forward_name));
    ...
    protected function createView($view_class_name, $backend, $forward_name, $forward_path)
    {
        return new $view_class_name($backend, $forward_name, $forward_path);
    }
    ...
    function getManagerClassName($name)
    {
        ...
        //   組み合わせたものが返される
        $manager_id = preg_replace_callback('/_(.)/', function ($m) { return strtoupper($m[1]); }, ucfirst($name));
    ...
    function getObjectClassName($name)
    {
        ...
        $name = preg_replace_callback('/_(.)/', function ($m) { return strtoupper($m[1]); }, ucfirst($name));
